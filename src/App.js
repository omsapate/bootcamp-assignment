...
function App() {
  ...
  const handleSubmit = async (event) => {
    ...
    if (isReturn) {
      const url2 = `/filterrecords?origin=${destCity}&destination=${originCity}&price=${price}&date=${retDate}`;
    ...
    // }
    if (response && response.data) {
    setRows([...new Set([...response.data])]);
    setReturnRows([...new Set([...res])]);
    }
    ...
    <div className="App">
      ...
      <section className="container-fluid" style={{ padding: "30px 20px" }}>
        ...
        <section
          ...
          style={{ float: "right", margin: "0px 0px", minWidth: "70%" }}
        >
          <section className="flight-search-info">
            ...
            <PageList style={{ margin: "10px", width: "100%" }} items={rows} isreturn={isReturn} returnList={returnRows}/>
          </section>
        </section>
      </section>
    </div>
